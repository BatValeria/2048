...
class GameForm(QMainWindow):
    def __init__(self, parent=None):
        ...
        self.initUi()
        # Define the background color of each number
    ...
    def initGameOpt(self):
        ...
        self.lbFont = QFont('SimSun', 12)  # label font
        self.lgFont = QFont('SimSun', 50)  # Logo font
        self.nmFont = QFont('SimSun', 36)  # Panel digital font
    ...
    def drawScore(self, qp):
        ...
        curScoreBoardMinW = 15 * 2 + scoreLabelSize  # The minimum width of the SCORE column
        bstScoreBoardMinW = 15 * 2 + bestLabelSize  # BEST column minimum width
    ...
    def drawBg(self, qp):
        ...
        qp.setBrush(QColor(187, 173, 160))
        qp.drawRect(15, 150, 475, 475)  # Draw the game area
    ...
    def drawTiles(self, qp):
        ...
        for row in range(4):
            for col in range(4):
                ...
                qp.drawRect(30 + col * 115, 165 + row * 115, 100, 100)  # Draw the background small squares of the numbers
                size = self.nmFont.pointSize() * len(str(value))  # Get the length of the number displayed in the current font
                # Adjust font size according to size
                while size > 100 - 15 * 2:
                ...
                # Display non-zero numbers
                if value == 2 or value == 4:
                    qp.setPen(QColor(119, 110, 101))  # Set the foreground color of 2 and 4 numbers
                else:
                    qp.setPen(QColor(255, 255, 255))  # Set the foreground color of other numbers
                if value != 0:
    ...
    def slideUpDown(self, isUp):
        ...
        for col in range(numCols):
            ...
            for row in range(numRows):
                if self.data[row][col] != 0:
                    cvl.append(self.data[row][col])  # Extract the non-zero elements in the column

            if len(cvl) >= 2:
                cvl = self.merge(cvl)  # Combine the same numbers

            # Fill 0 according to the moving direction
        ...
        return oldData != self.data  # Return whether the data has changed
    ...
    def isGameOver(self):
        ...
        copyData = copy.deepcopy(self.data)  # First temporarily store data values
        ...
        if not self.slideUpDown(True) and not self.slideUpDown(False) and \
            ...
            flag = True  # Can no longer move in all directions
        self.curScore = curScore
        if not flag:
            self.data = copyData  # Can still be moved, then restore the original data
        return flag
